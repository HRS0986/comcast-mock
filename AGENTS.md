...
tickets         (id, title, description, category_id, sub_category_id, status)
investigations  (id, ticket_id, run_id, mcp_tools, inputs)
```
...
Static reference data, backs `categories` / `sub_categories` (10 categories × 5 sub-categories = 50 possible, but POC only seeds Cat-1.1 and Cat-1.2 with mock ticket data).
...
Backs the `investigations` table: `id, ticket_id, run_id, mcp_tools, inputs`. Logs which tools were called and with what inputs on a given run.
...
```
- `GET /investigations?ticket_id=tkt-...`
...
{
  "total_tickets": 30,
  "by_category": { "cat-1.1": 20, "cat-1.2": 10 }
}
```
...
2. Where does `run_id` come from — generated by Kaya per execution, or by this backend on first investigation write?
